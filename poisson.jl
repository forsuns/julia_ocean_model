# calculate right-hand side of Poisson equation
	...
	function Poisson(nw,nx,ny,nz,dx,dy,dz,nt,dt,dtin,g,rho0,omega,epsilon,f,u,v,w,un,vn,wn,uintegral,vintegral,diffu,diffv,diffw,
...
######### start external mode(testing, not used)
		for nrep = 1:nt
			...
			SorLoop(nx,ny,nz,dx,dy,dz,dt,rho0,dry,g,q,dq,atop,abot,ae,aw,as,an,atotal,drdx,drdy,drdz,
...
######## end external mode
	...
	function SorLoop(nx,ny,nz,dx,dy,dz,dt,rho0,dry,g,q,dq,atop,abot,ae,aw,as,an,atotal,drdx,drdy,drdz,
							qstar,ustar,vstar,wstar,u,v,w,un,vn,wn,uintegral,vintegral,omega,epsilon,ib)
	...
	# wet and dry cell setup. if dry=0 is wet. else dry~=0 is dry.
	 	@inbounds for j = 2:ny+1, i = 2:nx+1
 				...
 				if (depth[i,j] <= 0.0) 
					for k = 2:nz+2	# this point is very important nb+2, nz+2
						dry[i,j,k] = 1
					end
				else
					for k = 2:ib[i,j]
 				...
 				if ( (ele[i,j] - q[i+1,j,1]/(rho0*g)) < 0 )&&( dry[i+1,j,2]==0.0 ) dry[i,j,2] = 0.0 end
 				if ( (ele[i,j] - q[i-1,j,1]/(rho0*g)) < 0 )&&( dry[i-1,j,2]==0.0 ) dry[i,j,2] = 0.0 end
 				if ( (ele[i,j] - q[i,j+1,1]/(rho0*g)) < 0 )&&( dry[i,j+1,2]==0.0 ) dry[i,j,2] = 0.0 end
	 			if ( (ele[i,j] - q[i,j-1,1]/(rho0*g)) < 0 )&&( dry[i,j,2]==0.0 ) dry[i,j,2] = 0.0 end
	 	end
